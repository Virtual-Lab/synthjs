...
//   in a div with id = "synth" or if not present as childobject of "body"
// ------------------------------------------------------------------------------
...
Synth.set = {
  ...
  ['select', 'midi', 'setMidi', 'play', 'midi in: ']
  ], 
  Master : [
...
Midi.prototype.midiMap = function ( note, mapping ) {
  return mapping[note];
};

Midi.prototype.mapping = function ( note, begin, scale ) {

  var mapping = [];
  var index;

  for (var i = begin; i < scale.length; i++) {
    index++;
    mapping[i] = scale[index];
  }
  return mapping;
};
